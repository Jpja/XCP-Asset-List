...
max_date = '2017-12-31'   #only include if registered, traded and locked by this date
max_issued = 3000         #only include if less tokens ever been issued
include_divisible = True  #only include indivisible tokens if False
sort_by_complete = True   #sort by initial issuance if False
...
for asset in assets:
  ...
  #Date
  if asset[6]: #locked and traded
    if asset[7] <= max_date:
      ok_date = True
      
  #Issued
  if asset[9] <= max_issued:
    ok_supply = True
  
  #Divisibility
  if include_divisible == True:
    ok_div = True
  elif asset[8] == 0:
    ok_div = True
  
  #Output if all filters ok
  if ok_date and ok_supply and ok_div:
    out.append(asset)
  
  
#Sort by completed
if sort_by_complete:
  out = sorted(out, key=lambda l:l[7])


#Add header row
out.insert(0, ['asset', 'reg_date', 'locked', 'lock_date', 'traded', 'trade_date', 'match_all', 'match_all_date', 'divisible', 'tokens_issued', 'tokens_destroyed', 'token_supply'])

  
#Write .csv file
file = open('xcp_assets_filtered.csv', 'w+', newline ='')
with file:   
